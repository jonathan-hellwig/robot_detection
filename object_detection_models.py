...
class MultiClassJetNet(nn.Module):
    ...
    def _format_model_output(self, output):
        ...
        ouptut = output.reshape((-1, FEATURE_MAP_HEIGHT, FEATURE_MAP_WIDTH,
                                self.num_scalings, NUM_BOX_PARAMETERS + self.num_classes + 1))
        predicted_boxes = ouptut[:, :, :, :, 0:NUM_BOX_PARAMETERS].reshape(
            (batch_size, -1, NUM_BOX_PARAMETERS))
        predicted_class_logits = ouptut[:, :, :, :, NUM_BOX_PARAMETERS:].reshape(
