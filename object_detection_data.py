...
class Encoder:
    def __init__(self, default_scalings: torch.Tensor, feature_map_size: Tuple[int, int], num_classes: int, threshold: float = 0.5) -> None:
    ...
    def apply(self, target_boxes: torch.Tensor, target_classes: torch.Tensor):
        ...
        target_boxes_tl_br[:, 0:2] = target_boxes[:,
                                                  0:2] - target_boxes[:, 3:4] / 2
        target_boxes_tl_br[:,
                           2:4] = target_boxes[:, 0:2] + target_boxes[:, 3:4] / 2
        ious = calc_iou_tensor(target_boxes_tl_br,
                               self.default_boxes_tr_bl)
        _, best_dbox_idx = ious.max(dim=1)
        masked_ious = torch.logical_or((best_dbox_idx == torch.arange(
            0, self.default_boxes_tr_bl.size(0)).reshape(-1, 1)).T, ious > self.threshold) * ious
        ...
        dbox_classes = torch.zeros(
            self.default_boxes_xy_hw.size(0), dtype=torch.long)
    ...
    def _default_boxes_tr_bl(self):
        ...
        default_boxes = torch.zeros(
            (feature_map_height * feature_map_width * self.default_scalings.size(0), NUM_BOX_PARAMETERS))
        for i in range(default_boxes.size(0) // self.default_scalings.size(0)):
            selected_boxes_idx = range(
                i*self.default_scalings.size(0), (i+1)*self.default_scalings.size(0))
            top_left = torch.tensor(
                [(i % feature_map_width) / feature_map_width, (i // feature_map_width) / feature_map_height])
            default_boxes[selected_boxes_idx, 0:2] = top_left
            default_boxes[selected_boxes_idx,
                          2:4] = top_left + self.default_scalings
    ...
    def _default_boxes_xy_hw(self):
        ...
        for i in range(default_boxes.size(0) // self.default_scalings.size(0)):
            ...
            top_left = torch.tensor(
                [(i % feature_map_width) / feature_map_width, (i // feature_map_width) / feature_map_height])
            default_boxes[selected_boxes_idx,
                          0:2] = top_left + self.default_scalings/2
            default_boxes[selected_boxes_idx,
                          2:4] = self.default_scalings
        return default_boxes
...
class ObjectDetectionDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for label_string in label_strings:
            target_bounding_boxes.append(np.array(np.fromstring(
                label_string[1:], sep=' ', dtype=np.float32)))
            target_classes.append(np.array(np.fromstring(
                label_string[0], sep=' ', dtype=np.float32), dtype=np.int64))
...
class TransformedObjectDetectionDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        return self.images[idx], self.encoded_bounding_boxes[idx], self.target_masks[idx], self.encoded_object_classes[idx]
